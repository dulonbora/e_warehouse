...
class Brand__List__User_View extends Mdl_User_Item_List_View{
    ...
    public function loadAllPaggingByUser($user_id, $start, $limit) {
        $Q = "SELECT * FROM ". $this->_view_name ." WHERE CREATE_BY = ? AND USER_ID = ? ORDER BY ID DESC LIMIT " . $start . " ," . $limit . "  ";
        $result = null;
        try {
            $stmt = $this->conn->prepare($Q);
            $stmt->bindParam(1, $user_id);
    ...
    public function loadAllPaggingByCategoy($columnName, $values, $start, $limit) {
        ...
        $Q = "SELECT COUNT(*) AS TOTAL FROM " . $this->_view_name . " WHERE PACKING_ID = ? ";
        $total = 0;
        try {
            $stmt = $this->conn->prepare($Q);
    ...
    public function loadAllPaggingByItemId($itemId, $start, $limit) {
        $Q = "SELECT * FROM " . $this->_view_name . " WHERE PACKING_ID = ? ORDER BY ID DESC LIMIT " . $start . " ," . $limit . " ";
        $result = null;
        try {
            $stmt = $this->conn->prepare($Q);
            $stmt->bindParam(1, $itemId);
