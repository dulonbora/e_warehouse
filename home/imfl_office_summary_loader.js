...
function Ena_List_By_Status(id) {
    var url = "../home_request/imfl_list_by_status_io.php?i="+id+"&io=OS";
    $.ajax({
        type: "GET",
        url: url,
            ...
            if (parseInt(json.SUCCESS) === 1) {
                ...
                for (var i = 0; i < json.CONTENTS.length; i++) {
                    ...
                    output += "<tr>"
                            ...
                            + "<td>" + json.CONTENTS[i].NAME + "</td>"
                            + "<td>" + json.CONTENTS[i].TOTAL_CASE + " </td>"
                            + "<td class='text-center'><a href='view_imfl_details_full.php?i="+json.CONTENTS[i].ID+"' class='btn btn-danger btn-xs btn-rounded text-danger text brandList' id='" + json.CONTENTS[i].ID + "'>View</a></td>"
                            + "</tr>";
                    ok++;
                }
...
function Ena_List_By_Inward_Pending(id) {
    var url = "../home_request/imfl_list_by_status_io.php?i="+id+"&io=I";
    $.ajax({
        type: "GET",
        url: url,
        success: function (data) {
...
function Ena_List_By_Lost_Pending(id) {
    var url = "../home_request/imfl_list_by_status_io.php?i="+id+"&io=L";
    $.ajax({
        type: "GET",
        url: url,
        success: function (data) {
