...
if ($__mas->Insert() == 1) {
    ...
    if ($_action == 5) {
            ...
            $__Transfar_Item->loadValueById($_module_ids);
            
        if ($__Transfar_Item->UpdateStatus($_module_ids, $_action)) {
            
            $__Brand__List->setStatus($_action);
            $__Brand__List->setId($__Transfar_Item->getLoadUserItemIdById($_module_ids));
            
            if ($__Brand__List->UpdateStatus()) {
                $response["SUCCESS"] = 1;
            }
        }
    }
