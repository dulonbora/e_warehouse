...
if ($Result > 0) {
    ...
    foreach ($Result as $rows) {
        ...
        $Edict['USER_ID']                   = $rows['USER_ID']          == NULL ? "" : $Company->returnCompanyName($rows['USER_ID']);
        $Edict['TO_USER_ID']                = $rows['TO_USER_ID']       == NULL ? "" : $Company->returnCompanyName($rows['TO_USER_ID']);
        ...
        $Edict['CREATE_BY']                 = $rows['CREATE_BY']        == NULL ? "" : $Company->returnCompanyName($rows['CREATE_BY']);
        $Edict['MODIFY_ON']                 = $rows['MODIFY_ON']        == NULL ? "" : $rows['MODIFY_ON'];
        $Edict['MODIFY_BY']                 = $rows['MODIFY_BY']        == NULL ? "" : $Company->returnCompanyName($rows['MODIFY_BY']);
