...
function All_Officers_List() {
    var url = "../officres_request/officers_list_all_by_status.php?i="+status;
    $.ajax({
        type: "GET",
        url: url,
            ...
            if (parseInt(json.SUCCESS) === 1) {
                ...
                for (var i = 0; i < json.CONTENTS.length; i++) {
                    output += ""
                            ...
                            + "<td>" + json.CONTENTS[i].SUB_DIVISION + "</td>"
                            + "<td class='text-center'><a href='full_officers_details.php?i="+json.CONTENTS[i].ID+"' class='btn btn-danger btn-xs btn-rounded text-danger text brandList' id='" + json.CONTENTS[i].ID + "'>View</a></td>"
                            + "</tr>";
                    ok++;
                }
