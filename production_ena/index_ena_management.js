...
function addTooProduction() {
    $(document).on("click", "#addTooProduction", function (e) {
        e.preventDefault();
        var url = "../production_ena_request/ena_transfar_add.php";
        $.ajax({
            type: "POST",
            url: url,
...
function loadTableIndex() {
    var url = "../ena_management_ajax_request/load_brand_list.php?id="+userId+"&i="+page_no;
    $.ajax({
        type: "GET",
        url: url,
        success: function (data) {
            ...
            if (parseInt(json.SUCCESS) === 1) {
                ...
                for (var i = 0; i < json.CONTENTS.length; i++) {
                    ...
                    output += link
                            ...
                            + "<td class='text-center'><buttom class='fa icon-arrow-right text-danger text addToMyList' id='" + json.CONTENTS[i].ID + "'></buttom></td>"
                            + "<td class='text-center'><a href='../ena_management/Ena_Transection.php?i="+json.CONTENTS[i].ID+"' class='fa fa-bars text-danger text brandList' id='" + json.CONTENTS[i].ID + "'></a></td>"
                            + "</tr>";
