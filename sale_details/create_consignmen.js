...
function selectConsignMent() {
    ...
    $(document).on("change", "#CONSIGNMENT_TYPE", function () {
        ...
        $.ajax({
            type: "POST",
            url: url,
            data: {USER_FROM : oid, MODE : consignment_type, VCH_TYPE : 'SALE', LONGDATE : longDate }, // serializes the form's elements.
    ...
    $.ajax({
        ...
        success: function (data) {            
          ...
          //      $("#Cost_Price").html(COST_PRICE_PER_BOTTLE);
                FEE = 0;
                if(consignment_type==1 || consignment_type==4 || consignment_type==5 || consignment_type==5){FEE = j.TPF;}
