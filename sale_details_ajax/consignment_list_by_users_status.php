...
$TotalRow = $_transfar->TotalCountByUserByStatus($userId, $_Status, "SALE");
$limit = 20;
...
$Result = $_transfar->loadAllPaggingByUserIdByStatus($userId, $_Status, "SALE", $start, $limit);
...
if ($Result > 0) {
    ...
    foreach ($Result as $rows) {
        ...
        $Edict['USER_FROM_NAME']      = $rows['USER_TO']              == NULL ? "" : $_Company->returnCompanyName($rows['USER_FROM']) ;
        $Edict['USER_TO']           = $rows['USER_TO']              == NULL ? "" : $rows['USER_TO'] ;
        $Edict['USER_TO_NAME']      = $rows['USER_TO']              == NULL ? "" : $_Company->returnCompanyName($rows['USER_TO']) ;
        ...
        $Edict['STATUS_STR']        = $rows['STATUS']               == NULL ? 0 : $_transfar->StatusForOrder($rows['STATUS']) ;
